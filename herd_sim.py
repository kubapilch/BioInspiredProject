...
class Sheep:
  ...
  def get_alignment_influence(self, neighbors):
    ...
    average_direction = sum(neighbor.orientation for neighbor in neighbors) / len(neighbors)
  ...
  def get_separation_force(self, neighbors, sheep_radius):
    ...
    for neighbor in neighbors:
      distance = math.sqrt(((self.x - neighbor.x) % self.world.size)**2 + ((self.y - neighbor.y) % self.world.size)**2)
      if distance < sheep_radius and distance!=0:
        ...
        repulsive_force += (repulsion_direction[0] / distance, repulsion_direction[1] / distance)#direction and magnitude
        repulsive_dir+=math.atan2(repulsive_force[1],repulsive_force[0])#solely direction
  ...
  def get_fence_repulsion(self, fence_repulsion_factor):
    ...
    distance_to_fence = min(abs(self.x-self.world.size), abs(self.y - self.world.size))
    repulsion_strength = fence_repulsion_factor * (1 - distance_to_fence / self.world.size)
  ...
  def update(self):
    ...
    if self.state == "idle":
        ...
        if(rand>switch_p):
            self.state="walking"
        #idle to running
        p=math.exp((hull_area-running_threshold))*(1+running_mimicking_factor*(running_neighbors))
        ...
        if(rand>switch_p):
            self.state="idle"
        #walking to running
        p=(1+running_mimicking_factor*(running_neighbors))*math.exp(hull_area-running_threshold)
        switch_p=1-math.exp(-p)
        if(rand>switch_p):
            self.state="running"
      ...
      self.desired_orientation=self.update_orienation()
      d=math.sqrt(((self.x - hull_center[0]) % self.world.size)**2 + ((self.y - hull_center[1]) % self.world.size)**2)
      p=1/(math.exp(d)-0.8)*(1+stopping_mimicking_factor*idle_neighbors)
      switch_p=1-math.exp(-p)
      if(rand>switch_p):
          self.state="idle"
  ...
  def move(self):
    ...
    speed_ex=min(max((self.desired_speed-self.speed), -(max_speed_change*deltaT)), (max_speed_change*deltaT))
    self.speed=self.speed+deltaT*speed_ex
    orientation_ex=min(max((self.desired_orientation-self.orientation),-(max_dir_change*deltaT)), (max_dir_change*deltaT))
